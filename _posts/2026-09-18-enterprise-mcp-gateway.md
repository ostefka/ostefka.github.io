...
**Accept every audience your application answers to — and nothing else.** The single most expensive
recurring bug. The `aud` claim is *not* reliably the `api://` identifier URI you configured: a v2
...
> client-side error. We hit this three times in different disguises.
...
// AADSTS650057 — both REGISTRATION faults, not consent faults. We shipped that bug, and it sent us
// looking for an administrator to grant consent for a scope we had simply never added.
...
**Deliberately brief — this is the fastest-moving part and any detail here will age badly.** As of
today, the three surfaces are three different products with three separate registrations:

| Surface | What you register | Reversible? |
|---|---|---|
| Copilot Studio | a connector generated by its own wizard, with OAuth details | yes |
...
Three things that cost us real time, all likely to still be true for a while:

- **One identifier is a base64-encoded composite where the documentation implies a plain value.**
  Get it wrong and you get *"the request is malformed or incorrect"* with **zero** traffic reaching
  your server.
- **One setting is labelled "for testing only" and is the only one that worked.** The
  reassuring-sounding alternative — binding the registration to one specific app id — silently broke
  tool calls, with zero traffic reaching the server. We kept the organisation restriction and left
  the app restriction broad. We have **not** established that this is the supported production
  configuration, and you should settle that before a production rollout rather than concluding from
  this post that the warning label is wrong.
...
look in Solutions. We lost an afternoon to that.) Identity providers match callback URLs exactly, so
...
- **Classify per surface, not per product.** One platform was "backend decides" for its management
  ...
  product-level answer is how you tell a customer something untrue in good faith.
...
- Revoke one user's access and confirm the next call refuses. An access token already issued stays
  valid until it expires — know how long that window is before a customer asks.
...
- Check which registrations are **reversible** before publishing anything into a customer tenant. In
  the path we tested we could block a package but never found a supported way to delete one.
...
- Debug to a working state somewhere disposable, then publish once. Do not iterate in a customer's
  tenant.
